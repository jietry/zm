...
parser.add_argument('--model', default='bagem_s3dis_1', help='Model name [default: model]')
parser.add_argument('--log_dir', default='log_test_results', help='Log dir [default: log]')
...
parser.add_argument('--ckpt', default='../best_seg_model.ckpt', help='Log dir [default: log]')
parser.add_argument('--ckpt_meta', default=' ../best_seg_model.ckpt.meta', help='ckpt file [default: best_seg_model.ckpt.meta]')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            loss = MODEL.get_loss(pred, labels_pl, boundary_loss)
...
def eval(sess, ops, test_writer):
    ...
    for room_i,room in enumerate(room_name_list):
        ...
        #total_seen += np.sum((batch_label>0) )
        #loss_sum += loss_val
