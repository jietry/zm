...
parser.add_argument('--max_epoch', type=int, default=601, help='Epoch to run [default: 501]')
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    mAcc= np.mean(np.array(total_correct_class[1:])/(np.array(total_seen_class[1:],dtype=np.float)+1e-6))
    return acc
