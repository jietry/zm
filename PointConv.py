...
def gating_process(inputs,num_output_channels,scope,stride=1,padding='VALID',bn_decay=None,is_training=None):
  ...
  with tf.variable_scope(scope) as sc:    
    ...
    #outputs = tf_util.batch_norm_for_conv2d(outputs, is_training,bn_decay=bn_decay, scope='bn',data_format=df)
    ...
    return outputs
def feature_encoding_layer(xyz, feature, npoint, radius, sigma, K, mlp, is_training, bn_decay, weight_decay, scope, bn=True, use_xyz=True, boundary_label=None):
    ''' Input:
    ...
    with tf.variable_scope(scope) as sc:
        ...
        if boundary_label is None:
            ...
            #print('boundary_label',boundary_label.shape)
        ...
        grouped_feature =nonlinear_transform(grouped_feature,[mlp[-1]],scope='grouped_feature',is_training=is_training,bn_decay=bn_decay,weight_decay=weight_decay)
...
def feature_decoding_layer_depthwise(xyz1, xyz2, points1, points2, radius, sigma, K, mlp, is_training, bn_decay, weight_decay, scope, bn=True, use_xyz = True, boundary_label=None):
    ''' Input:                                      
    ...
    with tf.variable_scope(scope) as sc:
        ...
        new_points = tf_util.reduce_sum2d_conv(new_points, axis = 2, scope = 'fp_sumpool', bn=True,
                                        bn_decay = bn_decay, is_training = is_training, keep_dims = True)    
